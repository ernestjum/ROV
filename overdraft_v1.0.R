...
## macro data used for forecasting and sensitivity testing
...
###########################################################
###########################################################
## calculate pred losses MRMD

## number of accounts per quarter
num_acts1<-as.data.frame(dev_fcst_train[c("Quarterly_Num_Acct")])
num_acts<-ts(as.matrix(num_acts1))


## predicted losses per account with logit
p_Lt<-as.data.frame(pred_Lt$pred_loss_per_act_Lt)
p_Lt<-as.matrix(p_Lt)

## predicted losses per account with simple difference
p_s<-as.data.frame(pred_s$pred_loss_per_act_s)
p_s<-as.matrix(p_s)

p_GAM<-as.matrix(pred_GAM)

## calculate pred losses
pred_losses_Lt<-as.data.frame(num_acts*p_Lt)
colnames(pred_losses_Lt)<-c("pred_loss_Lt")

pred_losses_s<-as.data.frame(num_acts*p_s)
colnames(pred_losses_s)<-c("pred_loss_s")

pred_losses_GAM<-as.data.frame(num_acts*p_GAM)
colnames(pred_losses_GAM)<-c("pred_loss_GAM")
...
pred_and_act<-as.data.frame(cbind(date, actual_loss_per_act, pred_Lt, pred_GAM, pred_s, dev_f, 
                                  pred_losses_Lt, pred_losses_s, pred_losses_GAM))
...
p1<-ggplot() + 
  geom_line(data = time_series_loss, aes(x = data_date, y = act_loss, color = "actual losses")) + 
  geom_line(data = time_series_loss, aes(x = data_date, y = pred_loss_Lt, color = "predicted losses logit diff.")) +
  geom_line(data = time_series_loss, aes(x = data_date, y = pred_loss_s, color = "predicted losses simple diff.")) +
  geom_line(data = time_series_loss, aes(x = data_date, y = pred_loss_GAM, color = "predicted losses GAM.")) +
  geom_line(data = time_series_loss, aes(x = data_date, y = pred_loss_dev_Lt, color = "predicted losses dev.")) +
  xlab('date') +
  ylab('quarterly loss')


ggsave(p1, file="plot_loss.pdf")
...
## plot the percentage Difference (may not be needed)
p2<-ggplot() + 
  geom_line(data = t1, aes(x = data_date, y = error_Lt_pct, color = "pct. error logit")) + 
  geom_line(data = t1, aes(x = data_date, y = error_s_pct, color = "pct. error simple trans.")) +
  geom_line(data = t1, aes(x = data_date, y = error_GAM_pct, color = "pct. error GAM")) +
  geom_line(data = t1, aes(x = data_date, y = error_svm_pct, color = "pct. error svm.")) +
  geom_line(data = t1, aes(x = data_date, y = error_dev_pct, color = "pct. error dev.")) +
  xlab('date') +
  ylab('Percentage Difference')

ggsave(p2, file = "Percentage_Error_plot.pdf")


## error from quaterly losses: no need for this as losses are reflected in 
# in the loss per account

## error from actual and estimated losses
t2<-time_series_loss
t2$err_Lt<-t2$act_loss -t2$pred_loss_Lt
t2$err_s<-t2$act_loss - t2$pred_loss_s
t2$err_dev<-t2$act_loss -t2$pred_loss_dev
err_loss<-t2[c("err_Lt", "err_s", "err_dev")]

write.csv(err_loss, file="Errors_from_Quarterly_Losses")

## percentage error
t2$err_Lt_pct<-((t2$act_loss -t2$pred_loss_Lt)/t2$act_loss)*100
t2$err_s_pct<-((t2$act_loss - t2$pred_loss_s)/t2$act_loss)*100
t2$err_dev_pct<-((t2$act_loss -t2$pred_loss_dev)/t2$act_loss)*100

err_loss_pct<-err_loss<-t2[c("err_Lt_pct", "err_s_pct", "err_dev_pct")]

write.csv(err_loss_pct, file="Percentage_Errors_Quarterly_Losse")

p3<-ggplot() + 
  geom_line(data = t2, aes(x = data_date, y = err_Lt_pct, color = "pct. error logit")) + 
  geom_line(data = t2, aes(x = data_date, y = err_s_pct, color = "pct. error simple trans.")) +
  geom_line(data = t2, aes(x = data_date, y = err_dev_pct, color = "pct. error dev.")) +
  xlab('date') +
  ylab('Pct. Error Qrt Diff.')

ggsave(p3, file = "Pct_Error_Qrt.pdf")
...
image(model_Lt, UR_Lt_Diff ~ CPI_RT )

contour(model_Lt, UR_Lt_Diff ~ CPI_RT)

persp(model_Lt, UR_Lt_Diff ~ CPI_RT, col="blue", zlab = "Pred. Loss Per Account Logit Diff.",
      contours = list(z="top", col="orange"), shade = 1)


image(model_s, UR_s_Diff ~ CPI_RT )

contour(model_s, UR_s_Diff ~ CPI_RT)

persp(model_s, UR_s_Diff ~ CPI_RT, col="blue", zlab = "Pred. Loss Per Account Simple Diff.",
      contours = list(z="top", col="orange"), shade = 1)
